...
import transformers
from trainer_seq2seq_qa_tamura import QuestionAnsweringSeq2SeqTrainer
...
def main():
    ...
    def preprocess_validation_function(examples):
        ...
        for i in range(len(model_inputs["input_ids"])):
            ...
            model_inputs["example_id"].append(examples["qid"][sample_index])
    ...
    def post_processing_function(
        ...
        # Decode the predicted tokens.
        # futuran
        #print(outputs.predictions[0].shape)
        # futuran end
        preds = outputs.predictions
        #preds = EvalLoopOutput(outputs)
        if isinstance(preds, tuple):
            preds = preds[0]
        decoded_preds = tokenizer.batch_decode(preds, skip_special_tokens=True)
        ...
        #print("decoded_preds = {}".format(decoded_preds))
        ...
        for example_index, example in enumerate(examples):
            # This is the index of the feature associated to the current example.
            feature_index = feature_per_example[example_index]
            predictions[example["qid"]] = decoded_preds[feature_index]
        ...
        references = [{"id": ex["qid"], "answers": ex[answer_column]} for ex in examples]

        # futuran
        import pprint
        pprint.pprint(formatted_predictions)
        print(type(formatted_predictions))
        predictions_text = []
        for prediction in formatted_predictions:
            predictions_text.append(prediction['prediction_text'].strip('<extra_id_0>'))
        output_prediction_file = os.path.join(training_args.output_dir, "generated_predictions.txt")
        with open(output_prediction_file, "w", encoding="utf-8") as writer:
            writer.write("\n".join(predictions_text[0:50]))
        # futuran end

        # Leo
        dataF = pd.DataFrame(predictions_text, columns=['predictions'])
        dataF.to_csv('generated_predictions.csv',encoding='utf-8-sig')
        #
